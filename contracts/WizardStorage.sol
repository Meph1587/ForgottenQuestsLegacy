...
contract WizardStorage {
    ...
    function getWizardaAffinities(uint256 id)
        public
        view
        returns (uint16[] memory)
    {
        ...
        if (t2 != 7777) {
            uint256 length = affinityT1.length;
            for (uint256 i = 0; i < affinityT2.length; i++) {
                affinitiesList[length + i] = affinityT2[i];
            }
        }

        if (t3 != 7777) {
            uint256 length = affinityT2.length;
            for (uint8 i = 0; i < affinityT3.length; i++) {
                affinitiesList[length + i] = affinityT3[i];
            }
        }

        if (t4 != 7777) {
            uint256 length = affinityT3.length;
            for (uint8 i = 0; i < affinityT4.length; i++) {
                affinitiesList[length + i] = affinityT4[i];
            }
        }

        if (t5 != 7777) {
            uint256 length = affinityT4.length;
            for (uint8 i = 0; i < affinityT5.length; i++) {
                affinitiesList[length + i] = affinityT5[i];
            }
        }
